...
public:
    int fillCups(vector<int>& amount) {
        ...
        while(a or b or c){
            //if only one type of water is left return time + remaining water 
            if(!a and !b and c) return time +c;
            if(!a and b and !c) return time +b;
            if(a and !b and !c) return time +a;

            //pick the two greatest
            //if one water is zero it will never be chosed because other two will be non zero
